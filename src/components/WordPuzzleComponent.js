...
export const WordPuzzleComponent = (props) => {
  ...
  const chosePath = (item) => {
    ...
    if (lastLetter.row === letter.row && lastLetter.column - 1 === letter.column) {
      result = "right2left"
    } else if (lastLetter.row === letter.row && lastLetter.column + 1 === letter.column) {
      result = "left2right"
    } else if (lastLetter.column === letter.column && lastLetter.row + 1 === letter.row) {
      result = "top2bottom"
    } else if (lastLetter.column === letter.column && lastLetter.row - 1 === letter.row) {
      result = "bottom2top"
    }
  ...
  const isSelected = (searched) => {
    ...
    if (selectedLetters.length > 0) {
      for (let i = 0; i < selectedLetters.length; i++) {
        ...
        if (searched.row === element.row && searched.column === element.column) {
          found = true
          break
        }
      }
